use crate::{Config, ScreenshotService, ScreenshotRequest, Priority, BatchProcessor, ProgressTracker};
...
impl CliRunner {
    ...
    pub async fn run(&self, command: Commands) -> Result<(), Box<dyn std::error::Error>> {
        match command {
            ...
            Commands::Server { port, bind, metrics, health } => {
    ...
    pub async fn run_batch(&self, options: BatchOptions) -> Result<(), Box<dyn std::error::Error>> {
        ...
        info!("Loaded {} URLs from {}", urls.len(), options.input.display());
        
        // Create output directory
        fs::create_dir_all(&options.output).await?;
        
        // Create requests
        let requests = self.create_requests(urls, RequestOptions { format: options.format, width: options.width, height: options.height, full_page: options.full_page, wait: options.wait, selector: None })?;
        ...
        info!("Batch processing completed. Success: {}, Errors: {}", success_count, error_count);
        Ok(())
    }
    
    pub async fn run_single(&self, options: SingleOptions) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub async fn show_health(&self, detailed: bool) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if detailed {
            ...
            for health in health_checks {
                println!("  Instance {}: {:?} - Screenshots: {}, Age: {:?}, Idle: {:?}, Failures: {}",
                         health.id, health.status, health.screenshot_count, 
                         health.age, health.idle_time, health.failure_count);
            }
        }
