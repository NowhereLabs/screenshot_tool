use crate::{BrowserPool, ScreenshotService, Metrics, HealthLevel, HealthStatus, HealthThresholds};
use std::sync::Arc;
...
impl SystemHealthChecker {
    ...
    pub async fn check_system_health(&mut self) -> HealthStatus {
        ...
        info!("Health check completed in {:?}: {:?}", check_duration, overall_health);
    ...
    async fn check_browser_pool_health(&self) -> HealthLevel {
        ...
        if stats.healthy_instances < self.thresholds.min_available_browsers {
            warn!("Browser pool health critical: only {} healthy instances", stats.healthy_instances);
        ...
        if failure_rate > 0.5 {
            error!("Browser pool health critical: failure rate {:.2}%", failure_rate * 100.0);
            return HealthLevel::Critical;
        } else if failure_rate > 0.2 {
            warn!("Browser pool health warning: failure rate {:.2}%", failure_rate * 100.0);
    ...
    async fn check_resource_health(&self) -> HealthLevel {
        ...
        if let Ok(memory_usage) = self.get_memory_usage() {
            if memory_usage > self.thresholds.max_memory_usage {
                error!("Resource health critical: memory usage {} MB", memory_usage / 1024 / 1024);
                return HealthLevel::Critical;
            } else if memory_usage > self.thresholds.max_memory_usage * 8 / 10 {
                warn!("Resource health warning: memory usage {} MB", memory_usage / 1024 / 1024);
                return HealthLevel::Warning;
            }
        }
        ...
        if let Ok(disk_usage) = self.get_disk_usage() {
            if disk_usage > 0.95 {
                error!("Resource health critical: disk usage {:.2}%", disk_usage * 100.0);
                return HealthLevel::Critical;
            } else if disk_usage > 0.85 {
                warn!("Resource health warning: disk usage {:.2}%", disk_usage * 100.0);
                return HealthLevel::Warning;
            }
        }
    ...
    pub async fn start_periodic_health_checks(&mut self, interval_duration: Duration) {
        ...
        loop {
            ...
            match health_status.overall {
                ...
                HealthLevel::Warning => {
                    warn!("System health: WARNING - Performance: {:?}, Resources: {:?}, Errors: {:?}",
    ...
    async fn handle_critical_health(&self) {
        ...
        for health in health_checks {
            if matches!(health.status, crate::InstanceStatus::Failed | crate::InstanceStatus::Unresponsive) {
...
impl HealthMonitor {
    ...
    async fn check_alerts(&mut self, health_status: &HealthStatus) {
        ...
        if health_status.performance == HealthLevel::Critical {
            self.create_alert(AlertType::PerformanceDegradation, "Performance is critically degraded".to_string());
        }
    }
